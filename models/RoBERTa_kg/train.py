...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', default=5, type=int, required=False)
    ...
    parser.add_argument('--load_dir', default='weights/med_rand_kg_2', type=str, required=False)
    parser.add_argument('--validate_load_dir', default='../../preprocessed_data/data_med_rand/validate_data.pkl', type=str, required=False)
    parser.add_argument('--train_load_dir', default='../../preprocessed_data/data_med_rand/train_data.pkl', type=str, required=False)
    parser.add_argument('--log_dir', default='log/train_med_rand_kg_2.txt', type=str, required=False)
    parser.add_argument('--val_epoch_interval', default=1, type=int, required=False)
    parser.add_argument('--last_epoch_path', default="weights/med_rand_kg_2/", type=str, required=False)    
    ...
    parser.add_argument('--finetune_kg', default= 'false', type=str, required=False)
    ...
    print('load success')
    f = open('model_params.txt','w')
    print('Model Parameters',file=f)
    # for name, param in model.named_parameters():
    #     print('\t' + name + '\t', list(param.size()),file=f)
    print(model,file=f)
    print('load success')
    ...
    finetune_check_kg = args.finetune_kg
    ...
    if finetune_check == 'true' or finetune_check == 'True':
        ...
        checkpoint = torch.load(PATH + 'bestmodel.pth', map_location='cuda:0')
        ...
        model.eval()
    f = open('model.txt','w')
    p = open('colakemodel.txt','w')
    # model.resize_token_embeddings(len(RobertaTokenizer.from_pretrained('roberta-base')) + 2)
    if finetune_check_kg == 'true' or finetune_check_kg == 'True':
        print("Initiating finetuning")
        print('mm',model.state_dict().keys())
        f.write(str(model.state_dict().keys()))
        states_dict = torch.load(os.path.join("/home1/deeksha/MedKGLM/src/models/RoBERTa_kg/pretrained_kg_covid/model.bin"))
        print('dd',states_dict.keys())
        p.write(str(states_dict.keys()))
        del states_dict['roberta.embeddings.position_embeddings.weight']
        del states_dict['roberta.embeddings.token_type_embeddings.weight']
