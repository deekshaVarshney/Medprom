...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', default=6, type=int, required=False)
    ...
    parser.add_argument('--decoder_dir', default='weights/med_rand_kg/bestmodel.pth', type=str, required=False)
    parser.add_argument('--test_load_dir', default='../../preprocessed_data/data_med_rand/test_data.pkl', type=str, required=False)
    parser.add_argument('--pred_save_dir', default='Results/med_rand_kg_new/pred.txt', type=str, required=False)
    parser.add_argument('--reference_save_dir', default='Results/med_rand_kg_new/reference.txt', type=str, required=False)
    parser.add_argument('--metric_save_dir', default='Results/med_rand_kg_new/scores.txt', type=str, required=False)
    parser.add_argument('--output_save_dir', default='Results/med_rand_kg_new/Out.csv', type=str, required=False)
    ...
    checkpoint = torch.load(decoder_path, map_location='cuda:6')
    ...
    for batch_id, batch in enumerate(test_dataloader):
        with torch.no_grad():
            ...
            past = model.roberta(encoder_input, mask_encoder_input)
            ...
            # print(sentence[0])
            predict = tokenizer.decode(sentence[0].tolist())
            encoder_input = encoder_input.squeeze(dim=0)
            encoder_input_num = (encoder_input != 0).sum()
            inputs = tokenizer.decode(encoder_input[:encoder_input_num].tolist())

            decoder_input = decoder_input.squeeze(dim=0)
            decoder_input_num = (decoder_input != 0).sum()

            reference = tokenizer.decode(decoder_input[:decoder_input_num].tolist())
