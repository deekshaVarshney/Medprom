...
class transformers_model(nn.Module):
    def __init__(self, config, hidden_size, vocab_size):
        ...
        encoder_config = RobertaConfig.from_json_file(self.config)
        # self.encoder = BertModel(encoder_config)
        self.roberta = RobertaModel(encoder_config)
    ...
    def forward(self, input_ids, mask_encoder_input, output_ids, mask_decoder_input):
        ...
        encoder_hidden_states = self.roberta(input_ids, mask_encoder_input)
        ...
        # print(out.size())
        # print(self.hidden_size, self.vocab_size)
        out = self.linear(out)
        return out
